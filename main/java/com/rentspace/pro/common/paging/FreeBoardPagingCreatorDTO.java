...
public class FreeBoardPagingCreatorDTO {
	private FreeBoardPagingDTO freeBoardPagingDTO; // 페이지번호와 행 개수 저장 객체
	...
	public FreeBoardPagingCreatorDTO(long rowAmountTotal, FreeBoardPagingDTO freeBoardPagingDTO) {
		this.freeBoardPagingDTO = freeBoardPagingDTO;
		this.rowAmountTotal = rowAmountTotal;
		this.pagingNumCnt = 10;
		
//계산된 끝-시작 페이징 번호:
		this.endPagingNum = (int) (Math.ceil(freeBoardPagingDTO.getPageNum() / (this.pagingNumCnt * 1.0)))
				* this.pagingNumCnt;
		this.startPagingNum = this.endPagingNum - (this.pagingNumCnt - 1);
		
//행 총수를 기준으로 한 실제 마지막 페이지 번호
		lastPageNum = (int) (Math.ceil((rowAmountTotal * 1.0) / freeBoardPagingDTO.getRowAmountPerPage()));
		...
		System.out.println("전달된 페이징 기본데이터-freeBoardPagingDTO: " + freeBoardPagingDTO.toString());
