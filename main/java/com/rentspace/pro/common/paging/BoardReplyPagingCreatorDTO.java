...
@Getter
@ToString
public class BoardReplyPagingCreatorDTO {

	private BoardReplyPagingDTO boardReplyPaging; // 페이지번호와 행 개수 저장 객체
	...
	private long replyTotalByFreeboard_no; // 전체 행 개수
	private int pagingNumCnt; // 화면 하단에 표시할 기본 페이지 번호 개수(10)
	private int lastPageNum; // 마지막 페이지 번호
	private List<BoardReplyVO> replyList;

	public BoardReplyPagingCreatorDTO(long replyTotalByFreeboard_no, BoardReplyPagingDTO boardReplyPaging,
			List<BoardReplyVO> replyList) {
		this.boardReplyPaging = boardReplyPaging;
		this.replyTotalByFreeboard_no = replyTotalByFreeboard_no;
		this.replyList = replyList;
		this.pagingNumCnt = 10;
		// 계산된 끝-시작 페이징 번호:
		this.endPagingNum = (int) (Math.ceil(boardReplyPaging.getPageNum() / (this.pagingNumCnt * 1.0)))
				* this.pagingNumCnt;
		this.startPagingNum = this.endPagingNum - (this.pagingNumCnt - 1);
		// 행 총수를 기준으로 한 실제 마지막 페이지 번호
		lastPageNum = (int) (Math.ceil((replyTotalByFreeboard_no * 1.0) / boardReplyPaging.getRowAmountPerPage()));
		...
		System.out.println("댓글-전달된 페이징 기본데이터-BoardReplyPagingDTO: " + boardReplyPaging.toString());
