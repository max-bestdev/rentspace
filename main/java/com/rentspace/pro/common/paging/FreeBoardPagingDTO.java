...
public class FreeBoardPagingDTO {
	...
	//따라서, 메서드 이름이 scope 필드에 대한 getter 형식(getScopeXX)을 지켜야 합니다.
	...
	public FreeBoardPagingDTO(int pageNum) {
		if (pageNum <= 0) {
			this.pageNum = 1;
		} else {
	...
	public FreeBoardPagingDTO(int pageNum, int rowAmountPerPage) {
		if (pageNum <= 0) {
			this.pageNum = 1;
		} else {
			this.pageNum = pageNum;
