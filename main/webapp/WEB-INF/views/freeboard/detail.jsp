...
<div id="page-wrapper">
    <div class="row">
        <div class="col-lg-12">
            <h3 class="page-header" style="white-space: nowrap;">Board - Detail: <c:out value="${board.freeboard_no}"/>번 게시물</h3>
        </div>
    </div>
<%-- 게시물 상세 표시 시작 --%>    
    <div class="row">
    ...
    <form id="frmSendValue"><!-- 폼을 추가 -->
        <input type='hidden' name='freeboard_no' id="freeboard_no" value='<c:out value="${board.freeboard_no}"/>'>
...
$("#btnToModify").on("click", function(){
	//location.href='${contextPath}/freeboard/modify?freeboard_no=<c:out value="${board.freeboard_no}"/>';  //주석처리
	frmSendValue.attr("action", "${contextPath}/freeboard/modify");
...
$("#btnToList").on("click", function(){
	//location.href="${contextPath}/freeboard/freeBoardList";  //  주석처리
	frmSendValue.find("#freeboard_no").remove();//  목록화면 이동 시, freeboard_no 값 삭제
    ...
    myCommentClsr.getCmtList(
        ...
        function(replyPagingCreator) { //ajax에서 실행할 callback 함수
            ...
            for (var i = 0, len = replyPagingCreator.replyList.length; i < len; i++) {
            	   str	+='<li class="left clearfix commentLi" data-freeboard_no="' + freeboard_noValue
